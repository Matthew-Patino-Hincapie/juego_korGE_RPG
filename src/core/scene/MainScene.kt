package com.tuempresa.pokemonlike.scene
...
class MainScene : Scene() {
    ...
    private fun showDialog(npc: NPC) {
        isDialogActive = true

        // Crear diálogo simple en pantalla
        val dialogBox = solidRect(width - 40, 120, Colors.WHITE) {
            position(20, height - 140)
            alpha = 0.9
        }

        val dialogText = text("${npc.name}: ${npc.message}") {
            position(40, height - 120)
            textSize = 16.0
            color = Colors.BLACK
        }

        val continueText = text("Presiona ENTER para continuar") {
            position(40, height - 60)
            textSize = 12.0
            color = Colors.GRAY
        }
